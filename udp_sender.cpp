...
UdpSender::UdpSender(std::uint64_t max_size,std::uint64_t delay, std::uint16_t from_port,
                     bool need_request, QString message,QMap<QByteArray, QByteArray> files_data, QObject* parent) : QObject(parent) {
...
void UdpSender::Send_Data() {
    ...
    message_id = 0;
    start_rand_id = Create_Random_Id();
...
void UdpSender::Split_File_Data() {
    ...
    qDebug() << package_data << "<БЫЛО Осторожно туть!";
    local_start += package_size;
}
...
void UdpSender::Send_Package() {
    qDebug() << package_data << "Осторожно туть!";
    udp_Socket->writeDatagram((message_header + system_header + package_data), QHostAddress::Broadcast, port);
}
