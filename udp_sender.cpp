...
UdpSender::UdpSender(std::uint64_t max_size,std::uint64_t delay, std::uint16_t from_port,QByteArray rand_id,
                     bool need_request, QString message,QMap<QByteArray, QByteArray> files_data,
...
UdpSender::~UdpSender() {
    ...
    local_start = NULL;
    package_data.clear();
    rand_generator.~mersenne_twister_engine();
    sending_address.clear();
}
